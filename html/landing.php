...
while ($row = mysqli_fetch_assoc($user_posts)) {
    ...
    echo '<div class="lower-section">
        ...
        <div class="comments-section">
            <h2 class="teko-font" text-align="center">All comments</h2>';

    while ($comm = mysqli_fetch_assoc($comment_fetch)) {
        $comm_cont = $comm['comment_content'];
        $comment_user_number = $comm['user_number'];
        $comment_user_fetch = mysqli_query($conn, "select * from users where user_number='$comment_user_number'");
        $comment_user_dp = mysqli_fetch_assoc($comment_user_fetch);
        

        echo '<div class="comment-sec">
            <div class="comment-dp">
                <img src="data:image/jpeg;base64,' . base64_encode($comment_user_dp['dp_image_data']) . '" alt="' . $comment_user_dp['dp_image_name'] . '" height="20px" width="20px">
            </div>
            <div class="comment-content">
                <div class="comment-user-info">
                <i><p>' . $comment_user_dp['user_name'] . '</p></i>
                </div>
                <p>' . $comm_cont . '</p>
                <hr>
            </div>
